...
RinResult rin_process_wait_v1(RinProcess process, uint64_t timeout_ns, int32_t* exit_code) {
    SIMPLE_CALL(RIN_SDK_LIBRARY_BASE, BASE_PROCESS_WAIT, exit_code, process,timeout_ns,0,0,0,0);
}
...
RinResult rin_thread_join_v1(RinThread thread, uint64_t timeout_ns, int32_t* exit_code) {
    SIMPLE_CALL(RIN_SDK_LIBRARY_BASE, BASE_THREAD_JOIN, exit_code, thread,timeout_ns,0,0,0,0);
}
#endif
...
RinResult rin_wait_many_v1(RinSliceV1 items, uint64_t timeout_ns, uint32_t* index) {
    SIMPLE_CALL(RIN_SDK_LIBRARY_IPC, IPC_WAIT_MANY, index, items.address,items.size,timeout_ns,0,0,0);
}
RinResult rin_shared_memory_create_v1(uint64_t size, uint32_t flags, RinSharedMemory* memory) {
    SIMPLE_CALL(RIN_SDK_LIBRARY_IPC, IPC_SHARED_MEMORY_CREATE, memory, size,flags,0,0,0,0);
}
RinResult rin_service_connect_v1(RinStringV1 name, RinChannel* channel) {
    SIMPLE_CALL(RIN_SDK_LIBRARY_IPC, IPC_SERVICE_CONNECT, channel, name.address,name.size,0,0,0,0);
}
RinResult rin_wait_set_create_v1(uint32_t flags, RinWaitSet* wait_set) {
    SIMPLE_CALL(RIN_SDK_LIBRARY_IPC, IPC_WAIT_SET_CREATE, wait_set, flags,0,0,0,0,0);
}
...
RinResult rin_service_register_v1(RinStringV1 name, uint32_t flags, RinService* service) {
    ...
    SIMPLE_CALL(RIN_SDK_LIBRARY_IPC, IPC_SERVICE_REGISTER, service, name.address,name.size,flags,0,0,0);
}
RinResult rin_service_accept_v1(RinService service, uint64_t timeout_ns, RinChannel* channel) {
    ...
    SIMPLE_CALL(RIN_SDK_LIBRARY_IPC, IPC_SERVICE_ACCEPT, channel, service,timeout_ns,0,0,0,0);
}
#endif
...
RinResult rin_file_watch_v1(RinStringV1 path, uint32_t events, RinFileWatch* watch) {
    SIMPLE_CALL(RIN_SDK_LIBRARY_FS, FS_FILE_WATCH, watch, path.address,path.size,events,0,0,0);
}
...
RinResult rin_file_io_result_v1(RinIoRequest operation, uint64_t* transferred) {
    SIMPLE_CALL(RIN_SDK_LIBRARY_FS, FS_FILE_IO_RESULT, transferred, operation,0,0,0,0,0);
}
...
RinResult rin_socket_create_v1(uint32_t domain, uint32_t type, uint32_t protocol, RinSocket* socket) { SIMPLE_CALL(RIN_SDK_LIBRARY_NET, NET_SOCKET_CREATE, socket, domain,type,protocol,0,0,0); }
...
RinResult rin_socket_send_v1(RinSocket socket, RinSliceV1 data, uint32_t flags, uint64_t* sent) { SIMPLE_CALL(RIN_SDK_LIBRARY_NET, NET_SOCKET_SEND, sent, socket,data.address,data.size,flags,0,0); }
RinResult rin_socket_receive_v1(RinSocket socket, RinSliceV1 data, uint32_t flags, uint64_t* received) { SIMPLE_CALL(RIN_SDK_LIBRARY_NET, NET_SOCKET_RECEIVE, received, socket,data.address,data.size,flags,0,0); }
RinResult rin_dns_resolve_v1(RinStringV1 host, RinStringV1 service, RinSliceV1 results, uint64_t* count) { SIMPLE_CALL(RIN_SDK_LIBRARY_NET, NET_DNS_RESOLVE, count, host.address,host.size,service.address,service.size,results.address,results.size); }
RinResult rin_tls_connect_v1(RinSocket socket, RinStringV1 server_name, uint32_t flags, RinTlsSession* session) { SIMPLE_CALL(RIN_SDK_LIBRARY_NET, NET_TLS_CONNECT, session, socket,server_name.address,server_name.size,flags,0,0); }
...
RinResult rin_tls_send_v1(RinTlsSession session, RinSliceV1 data, uint32_t flags, uint64_t* sent) { SIMPLE_CALL(RIN_SDK_LIBRARY_NET, NET_TLS_SEND, sent, session,data.address,data.size,flags,0,0); }
RinResult rin_tls_receive_v1(RinTlsSession session, RinSliceV1 data, uint32_t flags, uint64_t* received) { SIMPLE_CALL(RIN_SDK_LIBRARY_NET, NET_TLS_RECEIVE, received, session,data.address,data.size,flags,0,0); }
...
RinResult rin_clipboard_get_v1(uint32_t format,RinSliceV1 data,uint64_t* required) { SIMPLE_CALL(RIN_SDK_LIBRARY_GUI,GUI_CLIPBOARD_GET,required,format,data.address,data.size,0,0,0); }
RinResult rin_dialog_show_v1(RinStringV1 title,RinStringV1 message,uint32_t flags,uint32_t* result) { SIMPLE_CALL(RIN_SDK_LIBRARY_GUI,GUI_DIALOG_SHOW,result,title.address,title.size,message.address,message.size,flags,0); }
...
RinResult rin_image_decode_v1(RinMediaObject object,uint32_t format,RinSliceV1 output,uint64_t* required) { SIMPLE_CALL(RIN_SDK_LIBRARY_MEDIA,MEDIA_IMAGE_DECODE,required,object,format,output.address,output.size,0,0); }
RinResult rin_audio_decode_v1(RinMediaObject object,RinSliceV1 output,uint64_t* frames) { SIMPLE_CALL(RIN_SDK_LIBRARY_MEDIA,MEDIA_AUDIO_DECODE,frames,object,output.address,output.size,0,0,0); }
RinResult rin_video_decode_v1(RinMediaObject object,RinSurface target,uint64_t* timestamp_ns) { SIMPLE_CALL(RIN_SDK_LIBRARY_MEDIA,MEDIA_VIDEO_DECODE,timestamp_ns,object,target,0,0,0,0); }
#endif

#if !defined(RINSDK_SPLIT_BUILD) || defined(RINSDK_BUILD_CONFIG)
RinResult rin_config_open_v1(RinStringV1 domain,uint32_t flags,RinConfigStore* store) { SIMPLE_CALL(RIN_SDK_LIBRARY_CONFIG,CONFIG_OPEN,store,domain.address,domain.size,flags,0,0,0); }
RinResult rin_config_get_v1(RinConfigStore store,RinStringV1 key,RinSliceV1 value,uint64_t* required) { SIMPLE_CALL(RIN_SDK_LIBRARY_CONFIG,CONFIG_GET,required,store,key.address,key.size,value.address,value.size,0); }
